...
1. Watches for a session **starting** in either tool (Claude Code's `SessionStart` hook, opencode's `chat.system.transform` injection point) and, at that moment, indexes-if-stale the other tool's most recent session for the current [[glossary#project|project]] and surfaces a one-line breadcrumb (session count and recency) — the agent decides on its own when to call for a full recap.
...
5. Complements, rather than replaces, the curated `.agents/memory/` wiki (shared user-level `AGENTS.md` "Curated memory"). The wiki holds durable, human-gated knowledge; x-memory's index holds session history. The `snapshot` subcommand that previously wrote a regenerated Markdown summary per project is gone — see [[archived-threads#materializing-snapshots-superseded|archived threads]].

The user should be able to end a session in one tool — however it actually ends, cleanly or not — open the other, and have the new session already know enough to decide whether to pull more, without running any command by hand.
